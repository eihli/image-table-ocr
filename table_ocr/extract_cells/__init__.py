...
def extract_cell_images_from_table(image):
    ...
    bordered = cv2.copyMakeBorder(thresh_closed, 50, 50, 50, 50, cv2.BORDER_CONSTANT, None, 255)
    contours, heirarchy = cv2.findContours(
        ~bordered, cv2.RETR_LIST, cv2.CHAIN_APPROX_SIMPLE,
    )

    num_contours = num_contours_this_erosion = len(contours)
    MAX_ATTEMPTS = 100
    sentinel = 0
    eroded_thresh = cv2.erode(bordered, cv2.getStructuringElement(cv2.MORPH_CROSS, (3, 3)))
    while num_contours == num_contours_this_erosion and sentinel < MAX_ATTEMPTS:
        best_so_far = bordered.copy()
    ...
    # BOUNDING_RECTS
    contours, heirarchy = cv2.findContours(
        ~contoured_table, cv2.RETR_LIST, cv2.CHAIN_APPROX_SIMPLE,
    )
    ...
    for row in rows:
        ...
        for x, y, w, h in row:
            ...
            cell_bordered = cv2.copyMakeBorder(cell_image, 2, 2, 2, 2, cv2.BORDER_CONSTANT, None, 255)
